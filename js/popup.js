// popup.js - 팝업 관련 기능
import { currentLanguage, translations, currentCategory } from './main.js';
...
function updatePrice() {
    ...
    // 샷 추가 옵션이 선택된 경우 추가 요금 표시
    if (selectedOptions.shot === 'extra') {
        totalPrice += 1000;
        popupPrice.textContent = `₩ ${currentMenuItem.price.toLocaleString()} + ₩ 1,000 = ₩ ${totalPrice.toLocaleString()}`;
    } else {
        popupPrice.textContent = `₩ ${currentMenuItem.price.toLocaleString()}`;
    }
}
